...
namespace Supremes.Parsers
{
    ...
    public class TokenQueue
    {
        ...
        public string ChompBalanced(char open, char close)
        {
            ...
            bool inRegexQE = false; // regex \Q .. \E escapes from Pattern.quote()
            do
            {
                if (IsEmpty())
                {
            ...
            while (depth > 0);
            var @out = (end >= 0) ? queue.Substring(start, end - start) /*substring*/ : string.Empty;
            if (depth > 0)
            {
        ...
        /// <returns></returns>
        public static string EscapeCssIdentifier(string input)
        {
            StringBuilder output = StringUtil.BorrowBuilder();
            TokenQueue queue = new TokenQueue(input);
            while (!queue.IsEmpty())
            {
                if (queue.MatchesCssIdentifier(ElementSelectorChars))
                {
                    output.Append(queue.Consume());
                }
                else
                {
                    output.Append(ESC).Append(queue.Consume());
                }
            }
        ...
        private static readonly string[] ElementSelectorChars = new[]{"*|", "|", "_", "-"};
        ...
        private static readonly String[] CssIdentifierChars = {"-", "_"};
        
        private string ConsumeEscapedCssIdentifier(params string[] matches) {
            int start = pos;
            bool escaped = false;
            while (!IsEmpty()) {
            ...
            string consumed = queue.Substring(start, pos - start);
            return escaped ? Unescape(consumed) : consumed;
        }
        
        private bool MatchesCssIdentifier(params string[] matches) {
